<?php
    ...
    if (isset($_SESSION['nome']) && isset($_SESSION['idade']) && isset($_SESSION['sexo'])) {
        ...
        if(isset($_GET['inf'])) 
        {
            $sql_1 = "SELECT pergunta, $bot, $hum  FROM pergunta WHERE id = $id";
            ...
            if ($_GET['inf'] > 1) 
            {
                ...
                if(mysqli_num_rows($result) == 0) {
                    ...
                    $sql_register = "INSERT INTO entrevistado (nome, idade, sexo, status, id_pergunta, opiniao, resposta, nome_ia) 
                    VALUES ('$nome', '$idade', '$sexo', 1, '$id_resp',' $opiniao', '$resp', '$bot')";
                    mysqli_query($conn, $sql_register);

                } else {
                    $id_resp = mysqli_fetch_assoc($result)['id'];
                    $sql_register = "INSERT INTO entrevistado (nome, idade, sexo, status, id_pergunta, opiniao, resposta, nome_ia) 
                    VALUES ('$nome', '$idade', '$sexo', 0, '$id_resp', '$opiniao', '$resp', '$bot')";
                    mysqli_query($conn, $sql_register);
                }
            }
        } 
...
<body>
    ...
    <form action="questionario.php" method="get">
        ...
        <div class="opiniao">
            ...
            <textarea name="opiniao" id="opiniao" placeholder="Se quiser, deixe sua opinião" value=""></textarea>
        </div>
